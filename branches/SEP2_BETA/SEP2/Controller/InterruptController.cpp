...
void Sensor::activateInterrupts() {
	...
	i = (*h).getSetInterrupt();
	cout << "Sensor: PortB_write_Interrupt 0x" << hex << i << endl;
}
...
void Sensor::connectToHAL() {
	...
	interruptId = InterruptAttach(INTERRUPT_VECTOR_NUMMER, ISR, eventptr,
			sizeof(event), 0);
	if (interruptId == -1) {
	...
	int i = (*h).getSetInterrupt();
	cout << "Sensor: Interrupt 0x" << hex << i << " ready." << endl;
}
